    ...
    for year in years:
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False) as temp_file:
                ...
                print(f"Data download completed for {var_name} in {year}. File saved to: {temp_file_path}")
                ...
                if os.path.getsize(temp_file_path) > 0:
                    ...
                    s3_key = f"{var_name}/{year}.zip"
                    s3_client.upload_file(temp_file_path, BUCKET_NAME, s3_key)
                    print(f"File uploaded to S3 bucket {BUCKET_NAME} with key {s3_key}")
                else:
                    print(f"Temporary file for {var_name} in {year} is empty. No data was retrieved.")
